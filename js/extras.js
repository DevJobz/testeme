...
class ExtrasManager {
    ...
    buildSearchIndex() {
        ...
        this.searchIndex = questions.map(question => ({
            ...
            options: question.options?.map(opt => opt.toLowerCase()).join(' ') || ''
        }));
    }
        ...
        if (filtersDiv && !filtersDiv.querySelector('.search-bar')) {
            ...
            if (searchInput) {
                ...
                if (window.utils && window.utils.debounce) {
                    searchInput.addEventListener('input', window.utils.debounce(searchHandler, 300));
                } else {
    ...
    searchEverything(query) {
        ...
        this.searchIndex.forEach(item => {
            ...
            if (matches) {
                results.push({
                    ...
                    description: `Fonte: ${item.source} | ${item.answered ? 'Respondida' : 'Não respondida'}`,
                    category: 'Questão',
        ...
        const pages = [
            { id: 'dashboard', title: 'Dashboard', description: 'Visão geral e estatísticas', icon: 'fa-home' },
            { id: 'generator', title: 'Gerar Questões', description: 'Criar novas questões a partir de PDFs ou textos', icon: 'fa-magic' },
            { id: 'questions', title: 'Minhas Questões', description: 'Visualizar e responder questões', icon: 'fa-list' },
            { id: 'reports', title: 'Relatórios', description: 'Análises e gráficos de desempenho', icon: 'fa-chart-bar' },
            { id: 'profile', title: 'Perfil', description: 'Configurações pessoais e preferências', icon: 'fa-user' }
        ];
    ...
    showKeyboardShortcuts() {
        ...
        window.utils.showToast('info', 'Atalhos de Teclado', `
    ...
    // Favorites system
    loadFavorites() {
        this.favorites = JSON.parse(localStorage.getItem('questionai_favorites') || '[]');
    }
    ...
    addToFavorites(questionId) {
        if (!this.favorites.includes(questionId)) {
            ...
            this.saveFavorites();
            window.utils.showToast('success', 'Adicionado aos favoritos', 'Questão salva nos favoritos');
        }
    }
    ...
    removeFromFavorites(questionId) {
        ...
        this.saveFavorites();
        window.utils.showToast('info', 'Removido dos favoritos', 'Questão removida dos favoritos');
    }

    saveFavorites() {
        localStorage.setItem('questionai_favorites', JSON.stringify(this.favorites));
    }
    ...
    loadNotifications() {
        this.notifications = JSON.parse(localStorage.getItem('questionai_notifications') || '[]');
    }
    ...
    saveNotifications() {
        localStorage.setItem('questionai_notifications', JSON.stringify(this.notifications));
    }

    showNotificationToast(notification) {
        window.utils.showToast(notification.type || 'info', notification.title, notification.message);
    }

    scheduleNotifications() {
        // Check for daily practice reminder
    ...
    exportData() {
        const data = {
            ...
            preferences: JSON.parse(localStorage.getItem('questionai_preferences') || '{}'),
        ...
        a.href = url;
        a.download = `questionai-backup-${new Date().toISOString().split('T')[0]}.json`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);

        window.utils.showToast('success', 'Dados exportados', 'Backup criado com sucesso!');
    }
    ...
    importData(file) {
        ...
        reader.onload = (e) => {
            try {
                ...
                if (data.preferences) {
                    localStorage.setItem('questionai_preferences', JSON.stringify(data.preferences));
                }
                ...
                window.utils.showToast('success', 'Dados importados', 'Backup restaurado com sucesso!');
                
                // Refresh current page
                if (window.app.currentPage === 'questions' && window.questionsManager) {
                    window.questionsManager.loadQuestions();
                }
                
            } catch (error) {
                console.error('Import error:', error);
                window.utils.showToast('error', 'Erro na importação', 'Arquivo inválido ou corrompido');
            }
        };
