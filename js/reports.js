...
class ReportsManager {
    ...
    calculateTimeStats(questions) {
        if (questions.length === 0) return { average: 0, fastest: 0, slowest: 0 };
    ...
    renderDashboardStats() {
        ...
        this.updateStatCard('total-answered', this.reportData.answeredQuestions);
        this.updateStatCard('success-rate', `${this.reportData.successRate}%`);
        this.updateStatCard('total-documents', this.reportData.userStats.documentsProcessed || 0);

        // Update detailed stats
        this.updateStatCard('best-streak', this.reportData.userStats.bestStreak || 0);
        this.updateStatCard('avg-time', this.formatTime(this.reportData.userStats.averageTime || 0));
        this.updateStatCard('total-points', this.reportData.userStats.totalPoints || 0);
                    ...
                    tooltip: {
                        callbacks: {
                            label: function(context) {
                                ...
                                const difficulty = ['facil', 'medio', 'dificil'][context.dataIndex];
    ...
    renderProgressChart() {
        ...
        this.charts.progress = new Chart(ctx, {
            type: 'line',
            data: {
                labels: progressData.map(d => d.date.toLocaleDateString('pt-BR')),
                datasets: [{
                    label: 'Taxa de Acerto (%)',
    ...
    updateGamificationStats() {
        // Update gamification stats from gamificationManager
        if (window.gamificationManager) {
            ...
            this.updateStatCard('current-streak', gamificationStats.currentStreak);
    ...
    exportReportData() {
        if (!this.reportData) {
            window.utils.showToast('warning', 'Sem dados', 'Não há dados para exportar');
            return;
        }
        ...
        a.href = url;
        a.download = `relatorio_questionai_${new Date().toISOString().split('T')[0]}.json`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);

        window.utils.showToast('success', 'Relatório exportado', 'Dados salvos com sucesso');
    }
        ...
        if (weakestDifficulty) {
            ...
            if (stat.percentage < 50) {
                insights.push({
                    ...
                    message: `Questões de nível ${this.formatDifficulty(difficulty)} precisam de mais atenção (${stat.percentage}% de acerto).`
                });
            }
        }
