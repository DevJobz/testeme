...
class QuestionApp {
    ...
    login(userData) {
        ...
        this.navigateTo('dashboard');
        this.loadUserPreferences();
        this.showToast('success', 'Login realizado com sucesso!', `Bem-vindo, ${userData.name}!`);
    }
    ...
    navigateTo(page) {
        ...
        const protectedPages = ['dashboard', 'generator', 'questions', 'reports', 'profile', 'faq'];
        
        if (protectedPages.includes(page) && !this.isAuthenticated) {
            this.navigateTo('login');
            this.showToast('warning', 'Acesso negado', 'Faça login para acessar esta página');
            return;
        }
        ...
        switch (page) {
            case 'dashboard':
                if (window.reportsManager) {
                    window.reportsManager.loadDashboard();
                }
                break;
            case 'questions':
    ...
    toggleTheme(isDark) {
        ...
        // Save preference
        const preferences = JSON.parse(localStorage.getItem('questionai_preferences') || '{}');
        preferences.darkMode = isDark;
        localStorage.setItem('questionai_preferences', JSON.stringify(preferences));
    }
...
window.utils = {
    ...
    showToast: (type, title, message) => window.app.showToast(type, title, message),
