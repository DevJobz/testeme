...
class StorageManager {
    ...
    checkStorageAvailability() {
        try {
            ...
            window.utils.showToast('error', 'Erro de armazenamento', 'localStorage não está disponível');
            return false;
        }
    }
    ...
    handleStorageChange(event) {
        ...
        if (event.key === this.storageKeys.currentUser && !event.newValue) {
            if (window.app && window.app.isAuthenticated) {
                window.app.logout();
                window.utils.showToast('info', 'Sessão encerrada', 'Você foi desconectado em outra aba');
            }
        }
    }
    ...
    deleteQuestion(id) {
        ...
        return this.setItem(this.storageKeys.questions, filteredQuestions);
    }
    ...
    exportAllData() {
        try {
            ...
            a.href = url;
            a.download = `questionai_backup_${new Date().toISOString().split('T')[0]}.json`;
        ...
        return new Promise((resolve, reject) => {
            ...
            reader.onload = (e) => {
                try {
                    ...
                    // Import data
                    if (data.users) this.setItem(this.storageKeys.users, data.users);
                    if (data.questions) this.setItem(this.storageKeys.questions, data.questions);
                    if (data.sessions) this.setItem(this.storageKeys.sessions, data.sessions);
                    if (data.preferences) this.setItem(this.storageKeys.preferences, data.preferences);
    ...
    restoreBackup(backupId) {
        try {
            ...
            // Restore data
            if (backup.data.users) this.setItem(this.storageKeys.users, backup.data.users);
            if (backup.data.questions) this.setItem(this.storageKeys.questions, backup.data.questions);
            if (backup.data.sessions) this.setItem(this.storageKeys.sessions, backup.data.sessions);
            if (backup.data.preferences) this.setItem(this.storageKeys.preferences, backup.data.preferences);

            window.utils.showToast('success', 'Backup restaurado', 'Dados restaurados com sucesso');
            return true;
        } catch (error) {
            console.error('Error restoring backup:', error);
            window.utils.showToast('error', 'Erro na restauração', error.message);
            return false;
        }
    }
    ...
    handleStorageError(error) {
        if (error.name === 'QuotaExceededError') {
            window.utils.showToast('warning', 'Armazenamento cheio', 'Considere limpar dados antigos ou fazer backup');
        } else {
            window.utils.showToast('error', 'Erro de armazenamento', 'Problema ao salvar dados');
        }
    }
