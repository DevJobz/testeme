...
class GamificationManager {
    constructor() {
        this.achievements = [
            ...
            },
            {
                id: 'perfect_score',
                name: 'Perfeição',
                description: 'Obtenha 100% de acerto em uma sessão de 10 questões',
                icon: 'fas fa-star',
                points: 150,
                unlocked: false
            }
    ...
    checkAchievements(sessionStats = null) {
        ...
        // First question
        if (this.userStats.totalQuestions >= 1 && !this.isAchievementUnlocked('first_question')) {
            newAchievements.push(this.unlockAchievement('first_question'));
        }

        // Streak achievements
        if (this.userStats.currentStreak >= 3 && !this.isAchievementUnlocked('streak_3')) {
            newAchievements.push(this.unlockAchievement('streak_3'));
        }

        if (this.userStats.currentStreak >= 7 && !this.isAchievementUnlocked('streak_7')) {
            newAchievements.push(this.unlockAchievement('streak_7'));
        }

        // Total questions
        if (this.userStats.totalQuestions >= 50 && !this.isAchievementUnlocked('questions_50')) {
            newAchievements.push(this.unlockAchievement('questions_50'));
        }

        // Perfect score in session
        if (sessionStats && sessionStats.total >= 10 && sessionStats.correct === sessionStats.total && !this.isAchievementUnlocked('perfect_score')) {
            newAchievements.push(this.unlockAchievement('perfect_score'));
        }

        // Daily streak
        if (this.userStats.dailyStreak >= 7 && !this.isAchievementUnlocked('daily_streak_7')) {
    ...
    showLevelUpNotification(level) {
        window.utils.showToast('success', 'Level Up!', `Parabéns! Você alcançou o nível ${level}!`);
    }
    ...
    updateDashboardStats() {
        ...
        if (pointsElement) pointsElement.textContent = this.userStats.totalPoints;
        if (streakElement) streakElement.textContent = this.userStats.currentStreak;
    }
